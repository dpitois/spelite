...
describe("ontologyRepository", () => {
  ...
  describe("Semantic Search (10 Validation Examples)", () => {
    it("1. wizard level 3", async () => {
      ...
      expect(spells.every(s => s.level === 3 && s.classes.some(c => c.index === 'wizard'))).toBe(true);
    }, 15000);
    ...
    it("3. magicien", async () => {
      ...
      expect(spells.length).toBeGreaterThan(0);
      expect(spells.every(s => s.classes.some(c => c.index === 'wizard'))).toBe(true);
    }, 15000);
    ...
    it("6. sorcier", async () => {
      ...
      expect(spells.length).toBeGreaterThan(0);
      expect(spells.every(s => s.classes.some(c => c.index === 'sorcerer'))).toBe(true);
    }, 15000);

    it("7. foudre", async () => {
      const spells = await nlSearch("foudre", "fr");
      expect(spells.length).toBeGreaterThan(0);
      expect(spells.every(s => s.mechanics?.damage_type === 'lightning')).toBe(true);
    }, 15000);
    ...
    it("9. sauvegarde dexterite", async () => {
      ...
      expect(spells.length).toBeGreaterThan(0);
      expect(spells.every(s => s.mechanics?.save_ability === 'dex')).toBe(true);
    }, 15000);
